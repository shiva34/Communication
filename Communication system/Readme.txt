			REPORT/ReadMe.
#I had completed full design of BPSK using MORSE encoder at transmittion 
   and Demodulation and Decoding at Reciever except the WGN in the medium.
1)Could not add White Gaussian Noise to see SNR and make graphs, kindly consider only the part I have completed.
...
Note: the only part left is adding noise with some snr's and then plotting them with bit and word error rates.
Got some issues with toolboxes for that part will try to resolve them before lab component starts.